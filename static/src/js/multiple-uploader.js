  ...
  init({
    ...
    if (document.getElementById("max-upload-number"))
      document.getElementById(
        "max-upload-number"
      ).innerHTML = `Upload up to ${maxUpload} files (2mb/img)`;
          ...
          self.#multipleUploader.innerHTML += `
                <div class="image-container" data-image-index="${index}" id="mup-image-${index}" data-acceptable-image="${+isImageSizeAcceptable}" >
                    ...
                    <img src="${URL.createObjectURL(
                      this.files[index]
                    )}"  class="image-preview" alt="" name=""imgaa />
                </div>`;
          //

          // arr1.push(URL.createObjectURL( this.files[ index ]) )
          //   arr1.push(this.files[index]);

          //   console.log(arr1);
          dataTransfer.items.add(this.files[index]);
        //   console.log(dataTransfer.files);
          //   let files = localStorage.files;

          //   if (!files) files = localStorage.files = dataTransfer.files;

          let inin = document.getElementById("finalimg");
          inin.files = dataTransfer.files;
         
          // console.log(inin.files);
          //
